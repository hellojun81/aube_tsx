...
const EmblaCarousel: React.FC<PropType> = (props) => {
  ...
  useEffect(() => {
    const divInfo = divRef.current as HTMLDivElement;
    if (divInfo) {
  ...
  const renderFloorInfo = () => {
    switch (floor) {
      case 10:
        return
      case 1:
        return <div><h1>{floor}Floor</h1>
          차량 진입이 가능한 4.5미터의 층고를 자랑하는 1층은 370㎡의 넓은 공간에
          자연광 쇼윈도와 3.5미터에 달하는 대형 거울이 배치되어 있습니다.
          빌라 사보아의 문을 오마주한 대형사이즈에 정문은 공간감을 극대화하면서도 유니크하게 설계되었습니다.
          이 독창적인 디자인은 과거와 현재 실용성을 동시에 담아내고 있습니다.</div>;
      case 2:
        return <div> <h1>{floor}Floor</h1>
      ...
      case 4:
        return <div> <h1>Other Space</h1>
          폴딩도어로 전면 개방이 가능한 이 별채는 도로와 접해 있어 접근성이 뛰어납니다.
          스페인산 빈티지 타일과 노출 천장이 어우러져 차분하면서도 고풍스러운 분위기를 자아내며,
          외부 행인들의 호기심을 자극합니다.
        </div>;
      case 5:
        return <div> <h1>Stairs</h1>
          계단과 복도는 1950~70년대 영국과 동유럽에서 제작된 빈티지 조명으로 장식되어 있습니다.
          이 조명들은 공간에 빈티지 하면서도 절제된 감성을 연출합니다.
          빈티지 조명의 독특한 디자인은 고유의 역사와 매력을 담고 있어, .
          이동 공간을 과거와 현재가 조화를 이루어 더욱 감각적인 공간으로 연출됩니다
        </div>;
      case 6:
        return <div> <h1>OutSide</h1>
